...
func TestAuthorizerCheckAccess(t *testing.T) {
	...
	const (
		...
		aiSearchMethod  = "/memos.api.v1.AIService/SearchMemos"
	)
	...
	cases := []struct {
		...
		{"anonymous denied on AI search, open instance", openInstance, aiSearchMethod, nil, true},
		{"authenticated reaches AI search", privateInstance, aiSearchMethod, authenticated, false},
	}
	for _, c := range cases {
