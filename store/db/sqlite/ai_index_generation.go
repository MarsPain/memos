...
func (d *DB) PromoteAIIndexGeneration(ctx context.Context, promote *store.AIIndexGenerationPromotion) (bool, error) {
	...
	result, err := tx.ExecContext(ctx,
		"UPDATE `ai_index_generation` SET `state` = ?, `retired_ts` = 0, `updated_ts` = strftime('%s', 'now') WHERE `id` = ? AND `state` = ? AND `fingerprint` = ?",
	...
	if _, err := tx.ExecContext(ctx,
		"UPDATE `ai_index_generation` SET `state` = ?, `retired_ts` = ?, `updated_ts` = strftime('%s', 'now') WHERE `state` = ? AND `id` <> ?",
