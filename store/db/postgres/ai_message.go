...
func (d *DB) CreateAIMessageAttempt(ctx context.Context, attempt *store.AIMessage) (*store.AIMessage, error) {
	...
	if err := tx.QueryRowContext(ctx, "SELECT COALESCE(MAX(attempt), 0) + 1 FROM ai_message WHERE parent_id = $1", attempt.ParentID).Scan(&nextAttempt); err != nil {
...
func (d *DB) ListAIMessages(ctx context.Context, find *store.FindAIMessage) ([]*store.AIMessage, error) {
	...
	if find.ParentID != nil {
		where, args = append(where, "parent_id = "+placeholder(len(args)+1)), append(args, *find.ParentID)
	}
	if find.ClientRequestID != nil {
		where, args = append(where, "client_request_id = "+placeholder(len(args)+1)), append(args, *find.ClientRequestID)
	}
	if find.Status != nil {
		where, args = append(where, "status = "+placeholder(len(args)+1)), append(args, find.Status.String())
	}
...
func (d *DB) UpdateAIMessage(ctx context.Context, update *store.UpdateAIMessage) error {
	...
	if v := update.Status; v != nil {
		set, args = append(set, "status = "+placeholder(len(args)+1)), append(args, v.String())
	}
	...
	stmt := "UPDATE ai_message SET " + strings.Join(set, ", ") + " WHERE id = " + placeholder(len(args))
...
func insertAIMessage(ctx context.Context, tx *sql.Tx, message *store.AIMessage) (*store.AIMessage, error) {
	...
	stmt := "INSERT INTO ai_message (" + strings.Join(fields, ", ") + ") VALUES (" + placeholders(len(args)) + ") RETURNING id, created_ts, updated_ts"
